...
On the gate's `gate_passes()` returning False, `_process_account` breaks
the loop and calls `_handle_gate_failure` (`orchestrator.py:157`), which
delegates to `writeback.write_gate_failure_to_notion`. M1 fails when
`output["operates_in_scope"]` is False (no EU/UK/Norway/Switzerland/NA
operations).

**Run-once gates.** `module_01_gate` sets `Task.run_once = True`. The
orchestrator's `_maybe_skip_fresh` consults `runs.db` task_runs at
start-of-task; if a prior successful run exists for the (account, task)
pair, the gate is skipped and its cached `output_json` is replayed into
the context envelope + page-body writeback. Once it passes for an
account, it never runs again unless the task_run row is manually deleted
(or `--rerun module_01_gate:<account_substring>` is passed).
