...
- ✅ Phase 2a (2026-05-11): module 8 ad library + Apify integration
- ✅ Phase 2b (2026-05-11): module 2 strategic narrative + Pain Point Tags
- ✅ Citation rollout (2026-05-11): orchestrator-side per-section renumbering,
  applied to all 10 page-body modules
- ✅ Module 11 v1.5.0 (2026-05-11): Greenhouse ATS direct read +
  important-role open/close detection + location-aware hiring signal

**11 task modules registered** (`tasks/__init__.py`):
research_pass, module_01_gate, module_02_revenue_model, module_03_pain_points,
module_04_corporate_structure, module_05_structural_news, module_06_trigger_events,
module_07_creative_reality, module_08_ad_library, module_09_competitor_snapshot,
module_10_industry_pulse, module_11_hiring_signal.
Module 3 (pain_points) runs LAST because it synthesizes from modules 1, 2, 9 outputs.
...
| **v0.4.0 + Phase 2 (now)** | **~$0.20–0.30** | adds modules 3 (pain points) + 8 (ad library) + ATS; Apify free-tier covers tool cost |
...
- Per-prompt golden cases for modules 2-11 (currently only module_01_gate +
  a handful of others have goldens)
...
Module 3 follows the same contract — its prior `to_blocks` self-rendering
...
## Module 11 — ATS direct read + location-aware hiring signal
...
Greenhouse). Module 11 now uses three tools:
...
Module 11 schema has `creative_marketing_roles_in_scope_count` separate
...
- `module_02_revenue_model` — revenue model + customers + products
- `module_04_corporate_structure` — standalone/subsidiary/parent + PE
- `module_05_structural_news` — last-6mo events with constrained vocab
- `module_06_trigger_events` — funding / rebrand / agency switch / AI initiatives
- `module_07_creative_reality` — JD pain phrases + named agencies
- `module_09_competitor_snapshot` — top 3 direct competitors
- `module_10_industry_pulse` — last-60d category stories
- `module_11_hiring_signal` — hiring/downsizing via jobspy + Tavily
...
| `Pain Point Tags` | multi-select | module 2 | **agent-only** |
...
| `sister/child` | text (renamed from `Parent-Child` 2026-05-12) | module 2 (sister / child brands) | agent |
| `Parent` | text (renamed from `Name of Parent`) | module 2 (parent name / self if standalone-parent / empty) | agent |
| `Structure Notes` | text | module 2 | agent |
...
`Pain Point Tags` (agent-only — module 3):
...
- **Module 3 Parent self-name bug fix.** The model frequently classifies a
...
- **Module 3 placeholder-name fix.** Prompt v1.4.0 hardens the rule that
...
- **Module 8 90d window + Superside-relevance filter.** Prompt v1.4.0
  ...
  industry news.
- **Module 3 v3.0.0 — bullet layout.** Replaces the v2.x narrative-paragraph
...
1. `Overview` — modules 2 (revenue_model) + 4 (corporate_structure)
2. `Headcount` (subsection of Overview) — module 11 (hiring_signal) + important-role bullets
3. `Possible Pain Points` — module 3 (pain_points narrative + Pain Point Tags bullet)
4. `News` — modules 5 (structural_news) + 6 (trigger_events) + per-signal subsections
   (`### funding round`, `### industry movement`, `### hiring`, `### downsizing`, etc.)
   from modules 6, 10, 11
5. `Creative Posture` — module 7 (creative_reality)
6. `Ads Running` (subsection of Creative Posture) — module 8 (ad_library)
7. `Competitor Landscape` — modules 9 (competitor_snapshot) + 10 (industry_pulse)
...
- **2a**: module 8 (LinkedIn always; Meta/TikTok gated by audience).
- **2b**: module 2 — strategic narrative + Pain Point Tags (v2.0.0 →
  v2.2.0 = narrative-output style, no cold-email voice, no module-name
  citations, no horoscope filler).
- **2c** (not originally planned, added 2026-05-11): module 11 v1.5.0 —
...
- **Phase B**: per-task `detect_events(prev, curr)` hook on Task base. Each
  module owns its own diff rule (Module 05 structure_note severity +
  buying_implication transitions; Module 06 trigger_details URL/summary
  diff; Module 11 important_roles_recently_closed + new Tier-1 opens;
  Module 04 structure_type change; Module 03 timing-shift pain tags).
...
- Optional: enrich `research_pass` with a "strategic positioning + growth
  direction" section if module 2 narratives feel thin on smaller-profile
